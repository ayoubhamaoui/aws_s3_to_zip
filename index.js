...
exports.handler = async (_req, _ctx, _cb) => {
    ...
    var _list = await Promise.all(_keys.map(_key => new Promise((_resolve, _reject) => {
            s3.getObject({Bucket:'alphaflink', Key:_key}).promise().then(_data => _resolve( { data: _data.Body, name: `${_key.split('/').pop()}` } ));
        }
    ))).catch(_err => { throw new Error(_err) } );

    await new Promise((_resolve, _reject) => { 
        var _myStream = streamTo('alphaflink', 'fileName.zip');		//Now we instantiate that pipe...
